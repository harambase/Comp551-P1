        ...
        if X.shape[0] != len(y):
            ...
            for j in range(n_features):
                ...
                if likelihood_distribution[j] >= 0:
                    ...
                    # print(type(xd_categories[0]))
                    # print(type(xd_count_0[0]))
                    # print(type(xd_count_1[0]))
                    # print('*',xd_count_0)
                ...
                for j in range(X.shape[1]):
                    ...
                    if likelihood_distribution[j] >= 0:
                        if X[i][j] in self.likelihood[j][0]:
                            ...
                            idx = c.index(X[i][j])
                            # print(type(self.likelihood[j][1][idx]))
                            # print(type(y0))
                        ...
                        x = float(X[i][j])
                        y0 = y0 - np.square(x - mu_0) / (2 * np.square(sigma_0)) - 0.5 * np.log(
                            2 * 3.14 * np.square(sigma_0))
                        y1 = y1 - np.square(x - mu_1) / (2 * np.square(sigma_1)) - 0.5 * np.log(
                            2 * 3.14 * np.square(sigma_1))
